...
def auto_invoice_monthly():
	...
	for subdomain in subdomain_list:
		...
		inv.total_user = int(subdomain.quota)
		inv.append("invoice_item",{
		...
		inv.total = plist.normal_price + int(get_price_per_quota())*(int(subdomain.quota)-int(plist.user_qty)) + total_module
		inv.owner = subdomain.user
		inv.discount = 0
...
def auto_invoice_yearly():
	...
	for subdomain in subdomain_list:
		...
		inv.total = int(plist.normal_price + int(get_price_per_quota())*(int(subdomain.quota)-int(plist.user_qty)) + total_module) * 12
		inv.owner = subdomain.user
		inv.discount = 0
